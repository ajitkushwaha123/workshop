...
import { ArrowRight, CheckCircle2, TrendingUp, IndianRupee, ShoppingBag } from "lucide-react";

const transformations = [
    {
        title: "From Food Cart to Restaurant",
        beforeImage:
            "/assets/transformation/ram's-pizza-before.png",
        afterImage:
            "/assets/transformation/ram's-pizza-after.png",
    ...
    },
    {
        ...
        beforeImage:
            "/assets/transformation/sales-before.png",
        afterImage:
            "/assets/transformation/sales-after.png",
...
export function FeatureCard({ title, description, image, badge, children, className }) {
    return (
        <motion.div
            whileHover={{ y: -8 }}
            ...
            <div className="relative z-10 p-7 pt-2">
                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="w-full">
                        ...
                        {description && <p className="mt-2 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">{description}</p>}
                        {children}
                    </div>
                </div>
            </div>
                            ...
                            <div className="mb-10 flex flex-col items-center text-center">
                                ...
                                <div className="flex flex-wrap justify-center gap-3">
                                    {item.points.map((point, i) => (
                                        <div key={i} className="flex items-center gap-2 rounded-full border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 px-3 py-1.5 text-xs font-medium text-zinc-600 dark:text-zinc-400">
                                            <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> {point}
                                        </div>
                                    ))}
                                </div>
                            </div>
                                ...
                                >
                                    <div className="mt-6 flex gap-3">
                                        {item.metrics.map((metric, i) => {
                                            ...
                                            return (
                                                <div key={i} className="flex-1 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-4">
